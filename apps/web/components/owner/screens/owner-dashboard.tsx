...
const TH = "px-4.5 py-2.5 text-left text-[11px] font-medium uppercase tracking-wide text-muted-foreground"
      ...
      <div className="grid grid-cols-1 gap-3.5 lg:grid-cols-5">
        ...
        <DwCard className="p-4.5 lg:col-span-2">
          ...
          <div className="mt-3">
            ...
            <div className="bg-muted relative h-3 overflow-hidden rounded-full">
              <div
                className="bg-paid-foreground absolute inset-y-0 left-0"
                style={{ width: `${collectionPct}%` }}
              />
            </div>
            <div className="mt-3.5 flex flex-wrap gap-4 text-xs">
              <DotLegend color="bg-paid-foreground" label="Paid" value={fmtMoneyShort(stats.collected)} />
              <DotLegend color="bg-partial-foreground" label="Partial" value={fmtMoneyShort(partialPaid)} />
              <DotLegend color="bg-overdue-foreground" label="Overdue" value={fmtMoneyShort(stats.overdue)} />
              <DotLegend color="bg-muted-foreground/40" label="Unpaid" value={fmtMoneyShort(unpaidAmount)} />
            </div>
          </div>
          <div className="mt-4.5 border-t pt-4.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button variant="outline" size="sm" className="justify-start rounded-md">
                <RiMagicLine /> Generate June bills
              </Button>
              <Button variant="outline" size="sm" className="justify-start rounded-md" asChild>
                <Link href="/payments">
        ...
        <DwCard className="overflow-hidden lg:col-span-3">
          <div className="flex items-center justify-between px-4.5 pt-4 pb-2">
            ...
            <Link href="/bills" className="text-primary text-xs font-medium">
              View all bills →
            </Link>
          </div>
      ...
      <div className="grid grid-cols-1 gap-3.5 lg:grid-cols-5">
        <DwCard className="overflow-hidden lg:col-span-3">
          <div className="flex items-center justify-between px-4.5 pt-4 pb-2">
            ...
            <Link href="/payments" className="text-primary text-xs font-medium">
              View all →
            </Link>
          </div>
          <table className="w-full border-collapse">
            ...
            {PROPERTIES.map((p) => (
              <div key={p.id} className="flex flex-col gap-1.5">
                ...
                <div className="bg-muted h-1.5 overflow-hidden rounded-full">
                  <div
                    className="bg-paid-foreground/70 h-full"
...
function MethodPill({ method }: { method: string }) {
  ...
  return (
    <span
      className={
        isCash
          ? "bg-muted text-muted-foreground rounded-full px-1.5 py-0.5 text-[11px] font-medium"
          : "bg-accent text-primary rounded-full px-1.5 py-0.5 text-[11px] font-medium"
      }
    >
      {method}
    </span>
