...
type CreateOrganizationFormProps = {
  /** Plan chosen in the onboarding plan step. Display-only for now. */
  planId?: string
}
...
function CreateOrganizationForm({ planId }: CreateOrganizationFormProps) {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    try {
      // TODO: persist the selected plan once the backend supports it.
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-6">
      {selectedPlan && (
        <div className="bg-muted flex items-center justify-between rounded-md px-3.5 py-2.5 text-[13px]">
      ...
      <FieldGroup className="gap-5">
        <Field data-invalid={Boolean(form.formState.errors.name)}>
          <FieldContent>
            <FieldLabel htmlFor="org-name">Organization name<span className="text-red-500">*</span></FieldLabel>
